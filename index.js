...
app.get('/pokemon/:name', (req, res) => {
  ...
  if (results.length > 1) {
    ...
    res.status(404)
      .send(`Could not find information about ` +
            `${req.params.name[0].toUpperCase()}${req.params.name.slice(1)}` +
  ...
  let monFormat = [
    ...
    `They are weak against these type(s): ${mon.weaknesses.join(', ')}`,
    `They'll grow to ${mon.height} and weigh ${mon.weight} fully grown.`,
    mon.candy === "None" ?
      `${mon.name} don't really like candy.`
      : `${mon.name} love ${mon.candy}!`,
    mon.egg === "Not in Eggs" ?
      `They are not in eggs.`
      : `Their eggs take ${mon.egg} to hatch.`,
    mon.next_evolution ?
      `They'll evolve into ${mon.next_evolution[0].name}!`
      : `They don't have any evolutions.`,
  ];
...
app.get('/type/:type', (req, res) => {
  ...
  results.unshift(`These are all the Pokemon of ${req.params.type}` +
                  ` type in the first 151!`);
  res.send(results.join('<br>'));
});
...
app.get('/weakness/:weak', (req, res) => {
  ...
  results.unshift(`These are Pokemon weak against ` +
                  `${req.params.weak} types!`);
  res.send(results.join('<br>'));
});
